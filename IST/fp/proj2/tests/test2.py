...
class Testiadd(unittest.TestCase):
    ...
    def test_grupo_user(self):
        sys = Sistema()
        task1 = TarefaDependente(sys, "task1", 12.0)
        usr = Utilizador(sys, "ist99999", 'Jaquim Manel')
        usr1 = Utilizador(sys, "ist99998", 'Jaquim Nanel')
        grp = Grupo(sys, 'ist_grp99', 'Grupo do Jaquim')
        grp += usr
        grp += usr1
        task1.colaborador(grp)
        self.assertEqual("task1\n" , usr.tarefas())
        self.assertEqual("task1\n" , usr1.tarefas())

    def test_grupo_cicular(self):
        sys = Sistema()
        usr = Utilizador(sys, "ist99999", 'Jaquim Manel')
        usr1 = Utilizador(sys, "ist99998", 'Jaquim Nanel')
        usr2 = Utilizador(sys, "ist99997", 'Jaquim Matel')
        grp = Grupo(sys, 'ist_grp99', 'Grupo do Jaquim')
        grp1 = Grupo(sys, 'ist_grp98', 'Grupo do Jaquim man')
        grp2 = Grupo(sys, 'ist_grp97', 'Grupo do Jaquim no man')
        grp += usr
        grp += grp1
        grp1 += grp2
        grp1 += usr
        grp1 += usr2
        self.assertRaises(ValueError, grp2.__iadd__, grp)

    def test_diff_sys(self):
        sys1 = Sistema()
        sys2 = Sistema()
        task1 = TarefaDependente(sys1, "task1", 12.0)
        usr = Utilizador(sys2, "ist99999", 'Jaquim Manel')
        self.assertRaises(ValueError, task1.colaborador, usr)
